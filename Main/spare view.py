...
def add_to_cart(request, slug, *args, **kwargs):
    ...
    minimum_variation_count = Variation.objects.filter(itemy=item).count()
    ...
    for v in variations:
        order_item_qs = order_item_qs.filter(variation__exact=v
        )
...
This one is the latest but it was miserable
...
def add_to_cart(request, slug):
    ...
    for items in item_var:
        ...
        mar3 = int(mar2)
        mark += mar3
        print(mar3, "This is mars", type(mar3))

        print(items, "mohammed", items.id)
...
def add_to_cart(request, slug):
    ...
    if order_item and len(product_var) > 0:
        for product in product_var:
            order_item.variations.add(product)
