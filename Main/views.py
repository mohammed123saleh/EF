    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            if form.is_valid():
                ...
                #    #template_email = render_to_string('Main/email_template.html', {'name':first_name +' '+ last_name, 'order_num':order_num, 'customer_tel':Mobile, 'customer_address':shipping_address2, 'customer_gov':govs, 'customer_country':shipping_country} )
                    ##me = EmailMessage(
                    #    'Egypt Fabrics | Order Confirmation',template_email , settings.EMAIL_HOST_USER, [Email,], 
                        #'sales@egyptfabrics.com'
                    #)
...
def BedSheet(request):
    Beds = Item.objects.all().filter(Q(title__icontains='sheet')|
                                    Q(title__icontains='Bedsheet')|
                                    Q(title__icontains="Percale"))
...
def Petroleum(request):
    Petro = Item.objects.all().filter(Q(title__icontains='Petroleum')|
                                    Q(title__icontains='Petrol'))

    context = {
        'Petro':Petro
    }
    return render(request, "Main/Petroleum.html", context)    


def Hospitals(request):
    Hosp = Item.objects.all().filter(Q(title__icontains='Hospitals')|
